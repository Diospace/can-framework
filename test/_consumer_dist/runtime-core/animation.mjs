import { nextTick } from "./scheduler.mjs"; // Use the unified scheduler's nextTick
import { signal } from "../reactivity/signal.mjs"; // Keep reactivity imports
import { effect } from "../reactivity/effect.mjs";
import { onUnmounted } from "./apiLifecycle.mjs";
export async function enter(el, transitionClass) {
...
export async function leave(el, transitionClass, done) {
...
function parseEasing(easing) {
    if (typeof easing === 'function')
        return easing;
    if (presetEasings[easing])
        return presetEasings[easing];
...
export function animate(params) {
    const { targets, duration = 1000, delay = 0, endDelay = 0, easing = 'linear', loop = false, direction = 'normal', autoplay = true, update, complete, ...properties } = params;
    ...
    const instance = {
        ...
        get currentTime() {
            return animations[0]?.currentTime || 0;
        },
        set currentTime(t) {
...
export class Timeline {
    ...
    add(params, offset) {
        ...
        const anim = animate({ ...params, delay: (params.delay || 0) + startTime, autoplay: false });
...
function formatKeyframes(el, props) {
    ...
    const processValue = (key, val, isTo, currentBase) => {
        let result = val;
        ...
        if (typeof result === 'number') {
            if (transformKeys.includes(key)) {
                if (key.startsWith('translate') || key.startsWith('perspective') || key.startsWith('skew'))
                    result = result + 'px';
...
export function useTransition(el, name) {
    return {
        enter: () => enter(el, name),
        leave: () => leave(el, name)
    };
...
export function useMotion(target, keyframes, options = {}) {
    const animation = signal(null);
